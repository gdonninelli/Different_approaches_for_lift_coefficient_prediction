...
The optimizer comparison was performed exactly in the same way that the dropout one was done, using the winning topology from Section \ref{sec:04_topology_tuning} and the same cross-validation protocol. In the code, each candidate optimizer is specified through an \texttt{OptimizerRecipe} generated by helper functions (such as \texttt{Recipes::adam} or \texttt{Recipes::sgd\_momentum}). When \texttt{CrossValidator} evaluates each fold, this recipe dynamically instantiates a zero-initialized optimizer instance, ensuring that internal state variables are completely reset without state leakage across folds. The learning rate was fixed at $10^{-5}$ and all other hyperparameters were held constant, including the SIMM physics weight of $0.25$, batch size of 64 and a 100-epoch training budget.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{Images/Chapter04/optimizer/optimizer_history.png}
...
The diagnostic plots for the winning Adam optimizer reveal why it was so much more effective and stable than the alternatives. The key challenge is visualized in Figure \ref{fig:04_optimizer_gradient_magnitude}, which plots the gradient magnitudes for each trainable layer at different points in training.\\
...
The tuning campaign detailed in this chapter systemtically refined the surrogate model, providing several key insights into the architecture and training protocols required for high-fidelity aerodynamic prediction.\\
...
The regularization results were different from what might be expected for a large network. Both dropout and weight penalties increased the recorded validation error, and the physics-weight sweep also selected the zero-weight reference. The analysis repeatedly showed that these techniques primarily acted as "optimization noise", that hindered convergence, rather than providing a beneficial guard against overfitting.\\

% aggiungere parte sul perché preferibile lambda=0.1 invece di lambda 0

Ultimately, the final production model, combining the large topology with the refined training protocol and a $10^{-3}$ learning rate, produced an excellent final test MSE of $0.000487$.The 1200-epoch maximum was not exhausted: the run stopped at epoch 860, and the longer budget did not improve on the checkpoint already obtained with the 834-epoch budget. The high-angle test subset remained more difficult, with an MSE $2.374$ times the low-angle value, although it represented only 66 of the 429 test samples. This result reflects the recorded production recipe, the 90/10 ordinary split and validation-based restoration; it is a post-training test evaluation, not an unbiased test estimate for every tuning candidate.\\
