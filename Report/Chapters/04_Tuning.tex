...
This process begins with \texttt{LayerRecipe} instances, which define each layer not as an object, but as a set of instructions. Each recipe encapsulates two core functionals:
\begin{itemize}
...
After choosing the topology, optimizer, loss settings and activation, we studied the learning rate because it controls the scale and duration of the parameter updates. In Adam, the learning rate multiplies the bias-corrected adaptive direction; it is therefore not removed by the coordinate-wise denominator, even though that denominator adapts to the history of each parameter. A rate that is too small may lead to slow convergence, failing to reach an optimal solution within the fixed epoch budget. Conversely, a rate that is too large can cause the optimization to become unstable and diverge.\\

Beyond the learning rate's magnitude, its schedule, that represents how it changes over time, is also crucial. In addition to a constant rate, we tested step decay, cosine decay\footnote{This schedule has been studied as part of the SGDR family \citep{loshchilov2017sgdr}} and warm-up, that was introduced to address early optimization difficulties in large-minibatch stochastic-gradient training \citep{goyal2017accurate}. Those references motivate the schedule families tested here, but they do not predict the behaviour of this Adam run: the present experiment uses no warm restarts, a different optimizer and a different data set.\\
...
\begin{itemize}
    ...
    \end{cases}
    \end{equation*}
    where $W$ is the number of warm-up epochs. In the recorded grid, $E=100$, $W=5$, $\eta_{\mathrm{start}}=\eta_{\min}=10^{-5}$, and $\eta_{\mathrm{peak}}=10^{-1}$. Thus, the warm-up branch returns $10^{-5}$ at the first completed epoch and reaches $10^{-1}$ after 5 completed epochs (at the start of epoch 6).
\end{itemize}
\vspace{2ex}
...
\begin{itemize}
    \item[-] the top panel shows that all candidates with initial learning rates of $10^{-5}$ and $10^{-3}$ remain well-behaved with the $10^{-3}$ reaching the lowest overall validation errors. Among them, the constant $10^{-3}$ candidate achieves the lowest final validation error at epoch 100, despite noticeable mid-training fluctuations (e.g., around epochs 50 and 80). In contrast, the stepped $10^{-3}$ schedules exhibit smoother, monotonic trajectories, while the $10^{-5}$ candidates converge much more slowly and plateau early, confirming that their step size is insufficient for the $100$-epoch budget;
...
\end{itemize}
\vspace{2ex}
This analysis reinforces the conclusion that a learning rate of $10^{-3}$ represents an optimal trade-off, offering a balance between rapid convergence and numerical stability.\\

\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{Images/Chapter04/learning_rate/learning_rate_history.png}
    \caption{Sample-weighted validation histories at the ten-epoch checkpoints recorded for cross-validation. The upper panel contains the six candidates whose rates remain in the low-error scale, while the lower panel contains the five candidates that reach $10^{-1}$. The upper panel uses a linear vertical axis, whereas the lower panel uses a logarithmic scale to accomodate the large divergence in physical-unit MSE.}
...
The impact of these iterative improvements to the training protocol is documented in Table \ref{tab:04_final_training_trials}. The table chronologically presents the results of several training trials, clearly showing how the refinement of the batching and early stopping policies led to progressively better and more stable outcomes. All errors in the Table are physical-unit MSE values, where "budget" is the maximum configured epoch count, "completed" is the length of the diagnostic history and "selected" identifies the restored validation-minimum checkpoint.\\
...
The final production configuration synthesizes the findings of the tuning experiments. The model architecture consists of a $5\times5$ convolutional layer (stride $5$, $8$ challens) and a four-layer dense head of sizes ($1024$, $512$, $256$, $128$, $1$), using a LeakyReLU activation ($\alpha=0.05$) throughout. This configuration is trained using the Adam optimizer with a constant learning rate of $10^{-3}$. Based on our analysis, a small physics weight of $\lambda=0.10$ is included, while explicit regularizers like dropout, L1 and L2 penalties are omitted. The training is stabilized with gradient clipping at $\pm 1$ and executed with a balanced global batch size of 257 across 64 MPI ranks, governed by a "20/20" early stopping policy within a 1200-epoch budget.\\
